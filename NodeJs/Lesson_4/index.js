...
// Переменная для индивидуального iD, начинаем с 3, так как в файле было уже 3 пользователя:
let uniqueId = 3;
// Путь к файлу с пользователями, он нам пригодится, чтобы при перезапуске сервера данные не терялись:
...
app.get("/users/:id", (req, res) => {
  const users = JSON.parse(fs.readFileSync(pathFile));
  // Ищем в массиве по id нужного пользователя, params - в нем все параметры передаваемые при запросе. Все параметры имеют тип "строка", поэтому плюсом превращаем в число
...
app.put("/users/:id", (req, res) => {
  ...
  if (user) {
    ...
    // Перезаписываем новые данные в файл иначе изменения не сохранятся:
