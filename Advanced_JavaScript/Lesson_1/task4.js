...
// Обращаемся к коллекции по ключу(ученик), получаем значение(множество с уроками), через spread оператор распаковываем каждый урок и превращаем в массив, чтобы можно было воспользоваться методом массива - join, с его помощью задаем знак разделителя:
